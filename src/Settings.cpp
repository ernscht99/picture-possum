...
namespace possum {
    ...
    std::string Settings::render_tag_symbol(const std::string& tag_id) const {
    ...
    std::string Settings::render_tag_full(const std::string& tag_id) const {
        auto tag_iterator = tags.find(tag_id);
        if (tag_iterator != tags.end()) {
    ...
    Settings Settings::from_json(const QJsonObject& json) {
    ...
    std::string Settings::render_tag_name(const std::string& tag_id) const {
        auto tag_iterator = tags.find(tag_id);
        if (tag_iterator != tags.end()) {
