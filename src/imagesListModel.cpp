...
namespace possum {
    ...
    ImagesListModel::ImagesListModel(Settings settings, QObject *parent) : QAbstractTableModel(parent),
    ...
    ImagesListModel::ImagesListModel(Settings settings, std::map<std::string, std::unique_ptr<Image>> images,
                                     QObject *) : image_map(std::move(images)), settings(std::move(settings)),
                                                  unsaved_changes(false) {
